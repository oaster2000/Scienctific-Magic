...
public class ContainerSolarGenerator extends Container
{
    ...
    public ContainerSolarGenerator(InventoryPlayer playerInventory, TileEntitySolarGenerator furnaceInv)
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            for (int j = 0; j < 9; ++j)
            {
                this.addSlotToContainer(new Slot(playerInventory, j + i * 9 + 9, 8 + j * 18, 76 + i * 18));
            }
        }

        for (int k = 0; k < 9; ++k)
        {
            this.addSlotToContainer(new Slot(playerInventory, k, 8 + k * 18, 134));
        }	
    }
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index)
    {
        ...
        if (slot != null && slot.getHasStack())
        {
            ...
            {
                if (TileEntitySolarGenerator.isItemFuel(itemstack1))
                {
