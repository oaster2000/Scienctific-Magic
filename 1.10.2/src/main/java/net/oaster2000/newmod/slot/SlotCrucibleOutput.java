...
public class SlotCrucibleOutput extends Slot
{
    /** The player that is using the GUI where this slot resides. */
    private final EntityPlayer thePlayer;
    private int removeCount;

    public SlotCrucibleOutput(EntityPlayer player, IInventory inventoryIn, int slotIndex, int xPosition, int yPosition)
    {
    ...
    protected void onCrafting(ItemStack stack)
    {
        ...
        if (!this.thePlayer.worldObj.isRemote)
        {
            ...
            float f = CrucibleRecipies.instance().getSmeltingExperience(stack);
