...
public class MaceratorTileEntity extends TileEntityLockable implements ITickable, ISidedInventory {
	 ...
	 */
	private ItemStack[] maceratorItemStacks = new ItemStack[3];
	...
	public ItemStack getStackInSlot(int index) {
		return this.maceratorItemStacks[index];
	}
	...
	public void setInventorySlotContents(int index, @Nullable ItemStack stack) {
		boolean flag = stack != null && stack.isItemEqual(this.maceratorItemStacks[index])
				&& ItemStack.areItemStackTagsEqual(stack, this.maceratorItemStacks[index]);
		this.maceratorItemStacks[index] = stack;
	...
	public static void registerFixesMacerator(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists("Macerator", new String[] { "Items" }));
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		for (int i = 0; i < nbttaglist.tagCount(); ++i) {
			...
			if (j >= 0 && j < this.maceratorItemStacks.length) {
				this.maceratorItemStacks[j] = ItemStack.loadItemStackFromNBT(nbttagcompound);
			}
		}
		...
		this.currentItemBurnTime = getItemBurnTime(this.maceratorItemStacks[1]);
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		compound.setInteger("BurnTime", this.maceratorBurnTime);
		...
		for (int i = 0; i < this.maceratorItemStacks.length; ++i) {
			if (this.maceratorItemStacks[i] != null) {
				...
				this.maceratorItemStacks[i].writeToNBT(nbttagcompound);
	...
	public void update() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (this.isBurning() || this.maceratorItemStacks[0] != null) {
				if (!this.isBurning() && this.canSmelt()) {
					this.maceratorBurnTime = getItemBurnTime(this.maceratorItemStacks[1]);
					...
					if (this.isBurning()) {
						...
						if (this.maceratorItemStacks[1] != null && storage.getEnergyStored() == 0) {
							--this.maceratorItemStacks[1].stackSize;

							if (this.maceratorItemStacks[1].stackSize == 0) {
									this.maceratorItemStacks[1] = maceratorItemStacks[1].getItem()
											.getContainerItem(maceratorItemStacks[1]);
							}
						}
					}
				}
				...
				if (this.isBurning() && this.canSmelt()) {
					...
					if (this.cookTime == this.totalCookTime) {
						this.cookTime = 0;
						this.totalCookTime = this.getCookTime(this.maceratorItemStacks[0]);
						this.smeltItem();
						flag1 = true;
					}
				} else {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				MaceratorBlock.setState(this.isBurning(), this.worldObj, this.pos);
			}
		}
	...
	private boolean canSmelt() {
		if (this.maceratorItemStacks[0] == null) {
			...
			ItemStack itemstack = MaceratorRecipies.instance().getSmeltingResult(this.maceratorItemStacks[0]);
			if (itemstack == null)
				return false;
			if (this.maceratorItemStacks[2] == null)
				return true;
			if (!this.maceratorItemStacks[2].isItemEqual(itemstack))
				return false;
			int result = maceratorItemStacks[2].stackSize + itemstack.stackSize;
			return result <= getInventoryStackLimit() && result <= this.maceratorItemStacks[2].getMaxStackSize(); // Forge
																													// properly.
		}
	}
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			if (this.maceratorItemStacks[2] == null) {
				this.maceratorItemStacks[2] = itemstack.copy();
			} else if (this.maceratorItemStacks[2].getItem() == itemstack.getItem()) {
				this.maceratorItemStacks[2].stackSize += itemstack.stackSize; // Forge
																				// //
																				// items
			}

			if (this.maceratorItemStacks[0].getItem() == Item.getItemFromBlock(Blocks.SPONGE)
					&& this.maceratorItemStacks[0].getMetadata() == 1 && this.maceratorItemStacks[1] != null
					&& this.maceratorItemStacks[1].getItem() == Items.BUCKET) {
				this.maceratorItemStacks[1] = new ItemStack(Items.WATER_BUCKET);
			}
	...
	public boolean isItemValidForSlot(int index, ItemStack stack) {
		if (index == 2) {
			...
			ItemStack itemstack = this.maceratorItemStacks[1];
			return isItemFuel(stack)
					|| SlotMaceratorFuel.isBucket(stack) && (itemstack == null || itemstack.getItem() != Items.BUCKET);
		}
	}
	...
	public void clear() {
		for (int i = 0; i < this.maceratorItemStacks.length; ++i) {
			this.maceratorItemStacks[i] = null;
		}
	}
