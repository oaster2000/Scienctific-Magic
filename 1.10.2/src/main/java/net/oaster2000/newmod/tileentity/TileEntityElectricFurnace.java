...
public class TileEntityElectricFurnace extends TileEntityLockable implements ITickable, ISidedInventory {
	...
	public ItemStack getStackInSlot(int index) {
		return this.electricFurnaceItemStacks[index];
	}
	...
	public void setInventorySlotContents(int index, @Nullable ItemStack stack) {
		boolean flag = stack != null && stack.isItemEqual(this.electricFurnaceItemStacks[index])
				&& ItemStack.areItemStackTagsEqual(stack, this.electricFurnaceItemStacks[index]);
		this.electricFurnaceItemStacks[index] = stack;
	...
	public static void registerFixesElectricFurnace(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists("ElectricFurnace", new String[] { "Items" }));
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		for (int i = 0; i < nbttaglist.tagCount(); ++i) {
			...
			if (j >= 0 && j < this.electricFurnaceItemStacks.length) {
				this.electricFurnaceItemStacks[j] = ItemStack.loadItemStackFromNBT(nbttagcompound);
			}
		}
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		compound.setInteger("BurnTime", this.electricFurnaceBurnTime);
		...
		for (int i = 0; i < this.electricFurnaceItemStacks.length; ++i) {
			if (this.electricFurnaceItemStacks[i] != null) {
				NBTTagCompound nbttagcompound = new NBTTagCompound();
				nbttagcompound.setByte("Slot", (byte) i);
				this.electricFurnaceItemStacks[i].writeToNBT(nbttagcompound);
	...
	public boolean isBurning() {
		return storage.getEnergyStored() > 0 && electricFurnaceItemStacks[0] != null;
	}
	...
	public void update() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (this.isBurning() || this.electricFurnaceItemStacks[0] != null) {
				if (!this.isBurning() && this.canSmelt()) {
					this.electricFurnaceBurnTime = getItemBurnTime(this.electricFurnaceItemStacks[1]);
					...
					if (this.isBurning()) {
						...
						if (this.electricFurnaceItemStacks[1] != null && storage.getEnergyStored() == 0) {
							--this.electricFurnaceItemStacks[1].stackSize;

							if (this.electricFurnaceItemStacks[1].stackSize == 0) {
									this.electricFurnaceItemStacks[1] = electricFurnaceItemStacks[1].getItem()
											.getContainerItem(electricFurnaceItemStacks[1]);
							}
						}
					}
				}
				...
				if (this.isBurning() && this.canSmelt()) {
					...
					if (this.cookTime == this.totalCookTime) {
						this.cookTime = 0;
						this.totalCookTime = this.getCookTime(this.electricFurnaceItemStacks[0]);
						this.smeltItem();
						flag1 = true;
					}
				} else {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				ElectricFurnaceBlock.setState(this.isBurning(), this.worldObj, this.pos);
			}
		}
	...
	private boolean canSmelt() {
		if (this.electricFurnaceItemStacks[0] == null) {
			...
			ItemStack itemstack = FurnaceRecipes.instance().getSmeltingResult(this.electricFurnaceItemStacks[0]);
			if (itemstack == null)
				return false;
			if (this.electricFurnaceItemStacks[2] == null)
				return true;
			if (!this.electricFurnaceItemStacks[2].isItemEqual(itemstack))
				return false;
			int result = electricFurnaceItemStacks[2].stackSize + itemstack.stackSize;
			return result <= getInventoryStackLimit() && result <= this.electricFurnaceItemStacks[2].getMaxStackSize(); // Forge
																													// properly.
		}
	}
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			if (this.electricFurnaceItemStacks[2] == null) {
				this.electricFurnaceItemStacks[2] = itemstack.copy();
			} else if (this.electricFurnaceItemStacks[2].getItem() == itemstack.getItem()) {
				this.electricFurnaceItemStacks[2].stackSize += itemstack.stackSize; // Forge
																				// //
																				// items
			}

			if (this.electricFurnaceItemStacks[0].getItem() == Item.getItemFromBlock(Blocks.SPONGE)
					&& this.electricFurnaceItemStacks[0].getMetadata() == 1 && this.electricFurnaceItemStacks[1] != null
					&& this.electricFurnaceItemStacks[1].getItem() == Items.BUCKET) {
				this.electricFurnaceItemStacks[1] = new ItemStack(Items.WATER_BUCKET);
			}
	...
	public boolean isItemValidForSlot(int index, ItemStack stack) {
		if (index == 2) {
			...
			ItemStack itemstack = this.electricFurnaceItemStacks[1];
			return isItemFuel(stack)
					|| SlotElectricFurnaceFuel.isBucket(stack) && (itemstack == null || itemstack.getItem() != Items.BUCKET);
		}
	}
	...
	public void clear() {
		for (int i = 0; i < this.electricFurnaceItemStacks.length; ++i) {
			this.electricFurnaceItemStacks[i] = null;
		}
	}
