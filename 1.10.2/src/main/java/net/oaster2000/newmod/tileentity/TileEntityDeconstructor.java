...
public class TileEntityDeconstructor extends TileEntityLockable implements ITickable, ISidedInventory {
	 ...
	 */
	private ItemStack[] deconstructorItemStacks = new ItemStack[3];
	...
	public ItemStack getStackInSlot(int index) {
		return this.deconstructorItemStacks[index];
	}
	...
	public void setInventorySlotContents(int index, @Nullable ItemStack stack) {
		boolean flag = stack != null && stack.isItemEqual(this.deconstructorItemStacks[index])
				&& ItemStack.areItemStackTagsEqual(stack, this.deconstructorItemStacks[index]);
		this.deconstructorItemStacks[index] = stack;
	...
	public static void registerFixesDeconstructor(DataFixer fixer) {
		fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists("Deconstructor", new String[] { "Items" }));
	}
	...
	public void readFromNBT(NBTTagCompound compound) {
		...
		for (int i = 0; i < nbttaglist.tagCount(); ++i) {
			...
			if (j >= 0 && j < this.deconstructorItemStacks.length) {
				this.deconstructorItemStacks[j] = ItemStack.loadItemStackFromNBT(nbttagcompound);
			}
		}
		...
		this.currentItemBurnTime = getItemBurnTime(this.deconstructorItemStacks[1]);
	...
	public NBTTagCompound writeToNBT(NBTTagCompound compound) {
		...
		compound.setInteger("BurnTime", this.deconstructorBurnTime);
		...
		for (int i = 0; i < this.deconstructorItemStacks.length; ++i) {
			if (this.deconstructorItemStacks[i] != null) {
				...
				this.deconstructorItemStacks[i].writeToNBT(nbttagcompound);
	...
	public void update() {
		...
		if (!this.worldObj.isRemote) {
			...
			if (this.isBurning() || this.deconstructorItemStacks[0] != null) {
				if (!this.isBurning() && this.canSmelt()) {
					this.deconstructorBurnTime = getItemBurnTime(this.deconstructorItemStacks[1]);
					...
					if (this.isBurning()) {
						...
						if (this.deconstructorItemStacks[1] != null && storage.getEnergyStored() == 0) {
							--this.deconstructorItemStacks[1].stackSize;

							if (this.deconstructorItemStacks[1].stackSize == 0) {
									this.deconstructorItemStacks[1] = deconstructorItemStacks[1].getItem()
											.getContainerItem(deconstructorItemStacks[1]);
							}
						}
					}
				}
				...
				if (this.isBurning() && this.canSmelt()) {
					...
					if (this.cookTime == this.totalCookTime) {
						this.cookTime = 0;
						this.totalCookTime = this.getCookTime(this.deconstructorItemStacks[0]);
						this.smeltItem();
						flag1 = true;
					}
				} else {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				DeconstructorBlock.setState(this.isBurning(), this.worldObj, this.pos);
			}
		}
	...
	public void smeltItem() {
		if (this.canSmelt()) {
			...
			if(this.deconstructorItemStacks[0].getItem() == ModItems.matter){
				itemstack = new ItemStack(ModItems.stringUniverse);
			}else{
				itemstack = new ItemStack(ModItems.matter);	
			}

			if (this.deconstructorItemStacks[2] == null) {
				this.deconstructorItemStacks[2] = itemstack.copy();
			} else if (this.deconstructorItemStacks[2].getItem() == itemstack.getItem()) {
				this.deconstructorItemStacks[2].stackSize += itemstack.stackSize; // Forge
																				// //
																				// items
			}

			if (this.deconstructorItemStacks[0].getItem() == Item.getItemFromBlock(Blocks.SPONGE)
					&& this.deconstructorItemStacks[0].getMetadata() == 1 && this.deconstructorItemStacks[1] != null
					&& this.deconstructorItemStacks[1].getItem() == Items.BUCKET) {
				this.deconstructorItemStacks[1] = new ItemStack(Items.WATER_BUCKET);
			}

			--this.deconstructorItemStacks[0].stackSize;

			if (this.deconstructorItemStacks[0].stackSize <= 0) {
				this.deconstructorItemStacks[0] = null;
			}
		}
	}
	...
	public boolean isItemValidForSlot(int index, ItemStack stack) {
		if (index == 2) {
			...
			ItemStack itemstack = this.deconstructorItemStacks[1];
			return isItemFuel(stack)
					|| SlotDeconstructorFuel.isBucket(stack) && (itemstack == null || itemstack.getItem() != Items.BUCKET);
		}
	}
	...
	public void clear() {
		for (int i = 0; i < this.deconstructorItemStacks.length; ++i) {
			this.deconstructorItemStacks[i] = null;
		}
	}
