...
{	
    ...
    {
        boolean flag = stack != null && stack.isItemEqual(this.generatorItemStacks[index]) && ItemStack.areItemStackTagsEqual(stack, this.generatorItemStacks[index]);
    ...
    {
        fixer.registerWalker(FixTypes.BLOCK_ENTITY, new ItemStackDataLists("Generator", new String[] {"Items"}));
    }
    ...
    public void readFromNBT(NBTTagCompound compound)
    {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); ++i)
        {
            ...
            if (j >= 0 && j < this.generatorItemStacks.length)
            {
                this.generatorItemStacks[j] = ItemStack.loadItemStackFromNBT(nbttagcompound);
            }
        }
		...
		this.currentItemBurnTime = getItemBurnTime(this.generatorItemStacks[0]);
    ...
    public NBTTagCompound writeToNBT(NBTTagCompound compound)
    {
        ...
        compound.setInteger("BurnTime", this.generatorBurnTime);
        ...
        for (int i = 0; i < this.generatorItemStacks.length; ++i)
        {
            if (this.generatorItemStacks[i] != null)
            {
                ...
                this.generatorItemStacks[i].writeToNBT(nbttagcompound);
    ...
    public void update()
    {
        ...
        {
        	if (this.isBurning() || this.generatorItemStacks[0] != null)
            {
                if (!this.isBurning() && this.canGen())
                {
                    this.generatorBurnTime = getItemBurnTime(this.generatorItemStacks[0]);
                ...
                if (this.isBurning() && this.canGen())
                {
                    ...
                    {
                        this.cookTime = 0;
                        this.totalCookTime = this.getCookTime(this.generatorItemStacks[0]);
                        flag1 = true;
                    }
                }
                else
            ...
            {
                flag1 = true;
                GeneratorBlock.setState(this.isBurning(), this.worldObj, this.pos);
            }
    ...
    public boolean isItemValidForSlot(int index, ItemStack stack)
    {
            ...
            return isItemFuel(stack) || SlotGeneratorFuel.isBucket(stack) && (itemstack == null || itemstack.getItem() != Items.BUCKET);
    }
